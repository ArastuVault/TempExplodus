	...
	public class DamageOnTouch : MMMonoBehaviour
	{
		...
		protected virtual void OnCollideWithDamageable(Health health)
		{
			...
			if (health.CanTakeDamageThisFrame())
			{
				...
				float damage = MinDamageCaused;
				

				ApplyKnockback(damage, TypedDamages);

				DetermineDamageDirection();

				if (RepeatDamageOverTime)
				{
					_colliderHealth.DamageOverTime(damage, gameObject, Owner, InvincibilityDuration,
				...
				{
					_colliderHealth.Damage(damage, gameObject, Owner,InvincibilityDuration, InvincibilityDuration,
		...
		protected virtual void SelfDamage(float damage)
		{
			if (_health != null)
			{
				...
				_health.Damage(damage, gameObject, Owner, 0f, DamageTakenInvincibilityDuration, _damageDirection);
			}
